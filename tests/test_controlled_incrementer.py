...
from tests.gate_tests.incrementer import Test


class CIncrementerTestCase(unittest.TestCase):
    ...
    def test_control_register_for_3_qubits(self):
        self._test_control_register_for_n_qubits(3, 5, 2)
    ...
    def test_control_register_for_4_qubits(self):
        self._test_control_register_for_n_qubits(4, 12, 5)
    ...
    def test_control_register_for_5_qubits(self):
        self._test_control_register_for_n_qubits(5, 28, 10)
    ...
    def _test_for_n_qubits(self, n, x_values, g):
        params = [{'ctrl': 0b1, 'x': x, 'g': g} for x in x_values]

        test = Test(n, self)
        test.run_subtests(params)

    def _test_dirty_register_for_n_qubits(self, n, x, g_values):
        params = [{'ctrl': 0b1, 'x': x, 'g': g} for g in g_values]
        test = Test(n, self)
        test.run_subtests(params)

    def _test_control_register_for_n_qubits(self, n, x, g):
        ctrl_values = [0, 1]
